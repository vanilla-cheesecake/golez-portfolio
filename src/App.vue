...
async function loadVisitorAnalytics() {
  try {
    ...
    visitorAnalytics.value = await response.json()
...
async function countVisitor() {
  if (localStorage.getItem('portfolio-visitor-counted') === 'true') return
  ...
  try {
    ...
    // Leave the visitor unmarked so a later visit can retry after a temporary outage.
  }
}

function scheduleVisitorCount() {
  clearTimeout(visitorCountTimer)
  if (document.visibilityState !== 'visible') return
  if (localStorage.getItem('portfolio-visitor-counted') === 'true') return

  visitorCountTimer = window.setTimeout(countVisitor, 4000)
}
